...
print(paste("Strip: ", args[3]))
print(paste("Append: ", args[4]))
pathout = args[2]
prefix = unlist(strsplit(basename(input_name),".nii.gz",fixed=T))[1]
prefix = unlist(strsplit(prefix, args[3],fixed=T))[1]
print(paste("Prefix: ", prefix))
output_name = file.path(pathout, paste(prefix, args[4], ".nii.gz", sep=""))
print(paste("Output: ",output_name))
img = readnii(input_name)
img = rescale_img(img, min.val = -1024, max.val = 3071)
print("Finding skull...")
ss = CT_Skull_Strip(img, verbose = TRUE)
